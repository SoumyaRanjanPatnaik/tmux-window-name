...
def get_option(server: Server, option: str, default: Any) -> Any:
    ...
    if (
        option == "icon_style"
        and out[0]
        and not (out[0].startswith("'") or out[0].startswith('"'))
    ):
...
class Options:
    ...
    dir_programs: List[str] = field(
        default_factory=lambda: ["nvim", "vim", "vi", "git"]
    )
    ...
    def from_options(server: Server):
        ...
        if "icon_style" in fields_values and isinstance(
            fields_values["icon_style"], str
        ):
            try:
...
def get_program_icon(program_name: str, options: Options) -> str:
    ...
    icon = options.custom_icons.get(base_name) or DEFAULT_PROGRAM_ICONS.get(
        base_name, ""
    )
    ...
    logging.debug(
        f"Getting icon for program {program_name} (base_name: {base_name}) -> {icon!r}"
    )
    return icon
...
def get_current_program(
    running_programs: List[bytes], pane: TmuxPane, options: Options
) -> Optional[str]:
    ...
    for program in running_programs:
        ...
        if int(program[0]) == int(pane.pane_pid):
            ...
            program_name_stripped = re.sub(
                USR_BIN_REMOVER[0], USR_BIN_REMOVER[1], program_name
            )
            logging.debug(
...
def get_session_active_panes(session: Session) -> List[TmuxPane]:
    ...
    return [
        p
        for p in session.server.panes
        if p.pane_active == "1" and p.window_id in session_windows_ids
    ]
...
def rename_windows(server: Server, options: Options):
    with tmux_guard(server) as already_running:
        ...
        for p, display_path in exclusive_paths:
            ...
            logging.debug(
                f"processing exclusive_path: display_path={display_path} p.program={p.program}"
            )
